...
| # | Rule | Status | Source |
|---|---|---|---|
| 2.1.1 | **Bullish Step 1**: a meaningful visual break **and close above both** the 10 and 20 SMA. **Bearish**: break and close **below both**. **The close must clear the *further* MA by ≥10% of the parent bar's high-low range** (**D-29**) — for a bullish break `max(sma_10, sma_20)`, since *both* must be cleared. | P | `PC`, `M1 @ 09:47`, `M1 @ 10:03`; decision **D-29**, §11 |
...
| 2.1.4 | "Meaningful" is deliberately unquantified **by the material** — see the **J** note in §0. If you have to argue with yourself about it, pass. **This row still describes the source, and the source has not changed: `docs/evidence/census_meaningful.md` enumerates every passage on the term and none quantifies it.** The engine compiles **D-29**'s number, which is the human's decision layered on top — never a reading of `M1 @ 10:18`, which refuses in as many words. | J | `M1 @ 10:18`, `M1 @ 10:32`; number by decision **D-29**, §11 |
...
| 2.2.5a | **What an obvious base is, as the engine computes it (D-34).** The base is the **residual state**: after Step 1, price is either *trending* or *breaking out*, and anything else is a base. **Trending** = two consecutive **non-inside** candles extending the **same** way against the **parent bar** (5.2.8a) — both extremes up, or both extremes down. **Basing** = anything else, which makes a **sweep that reverses** (an outside bar) a basing candle rather than an end to the base. Inside candles are **skipped**, not treated as a pause in the trend, so a run of them mid-leg cannot open a base — and they do not count toward the floor either, so a run of them cannot **be** one (the 5.3.5a / **D-23** precedent, applied twice). The base is the trailing run of basing candles, it **ends at the candle before the one being tested**, its floor is **2 non-inside candles**, and it is cancelled if the leg resumes. **There is no compression test, no MA-proximity test and no "breakouts inside" test** — see **D-34** for why each was dropped rather than left unimplemented. **Status is P, not M:** the 2-candle floor is the one number the material does not give, and §0's legend puts a rule resting on a recorded number in P. | P | decision **D-34**, §11; `M1 @ 21:57` (duration is not a criterion), §2.2.4, **D-15**, **D-23**, **D-28** |
| 2.2.6 | The boundary may be *"clear support resistance or an obvious trend line"*; other consolidation shapes qualify *"without creating another setup system."* **The engine's default is now decided: the boundary is the base's *close* extreme on the Step 1 side** (**D-30**) — so choosing the base determines the line. The row stays **J** because §2.2.7's sloping case is retained and not implemented (**O-18**). | J | `M1 @ 11:31`; decision **D-30**, §11 |
| 2.2.7 | In `PC` the bearish selected boundary is drawn **horizontal** under the base; the bullish one is drawn as an **up-sloping line** over the base. A boundary is therefore not necessarily horizontal. **Retained on purpose when D-30 was taken** — a base-derived boundary is always horizontal, and this row is the standing exception to it. Corroborated independently by `SCALP @ 00:05:57`, live. **Not implemented: see O-18.** | J | `PC`, `SCALP @ 00:05:57` |
...
| 2.2.9 | If **two possible bases** appear, select the one that clearly controls the setup. *"If two boundaries look equally valid there is no clean step three yet"* — wait. **Under D-30 this reads on bases alone**, since the line follows from the base — which is how the source sentence already reads, moving from *"two possible bases"* to *"two boundaries"* without marking the change. **Under D-34 the engine never reaches this rule** — a residual base yields exactly one span per bar, so two candidates cannot be present to tie. Whether that satisfies the tie-break or drops it is **O-18**'s sibling, **O-20**. | J | `M1 @ 12:41`, `M1 @ 12:49`; decisions **D-30** and **D-34**, §11; open row **O-20**, §12 |
...
| 2.3.4 | **Confirmed Step 3** = a *meaningful close beyond the selected boundary*. **The close must sit beyond the boundary by ≥10% of the parent bar's high-low range** (**D-29**). The boundary is evaluated **at that bar**, so a sloping one (§2.2.7) needs no special case. | P | `PC`, `M1 @ 09:15`; decision **D-29**, §11 |
...
| 5.3.4a | **What ends the walk.** The order re-anchors candle by candle (5.3.5) until exactly one of **three** things happens: it **fills**; the setup is **invalidated without an entry** by any of §5.4.7a–c; or the **leg resumes past the Step 3 High/Low** (5.3.4b). There is no fourth outcome and **no timeout** — an unfilled order does not expire on bar count. | M | decisions **D-17** and **D-35**, §11 |
| 5.3.4b | **The leg resuming voids the walk.** If price trades beyond the **Step 3 High (long) / Step 3 Low (short)** while the order is still unfilled, the pullback that PTB belonged to is over and the working order is **cancelled**. It ends the **walk and nothing else**: no open position is touched, the count is **not** reset, and a **new pullback after that candle may open a continuation PTB** (§2.5, **D-21**). Trade-through, not close — the same intrabar test as the fill (§5.3.2) and break-even (§5.4.5), and the reason this is not one of §5.4.7's three, which are all close-based. | M | decision **D-35**, §11 |
...
fill, an invalidation, or the leg resuming past the Step 3 extreme (5.3.4b) does that.
...
| 7.1.4 | Do not trade **into** the 200 SMA on fast charts: *"on one minute chart, five minute chart, do not long into the 200 SMA."* **The material states the bullish case only** — nothing in `SCALP` addresses shorting into the 200 from below. **The mirror was decided on 2026-08-09 — see D-31** — so a short is blocked at or above the 200 exactly as a long is blocked at or below it. The bullish half is cited; **the bearish half is a decision, not a reading.** The **fast-chart scoping is cited and unchanged**: `SCALP @ 22:45` licenses the `50 → 200` long on a higher timeframe outright. | M (bullish) / P (mirror) | `SCALP @ 23:02`, `SCALP @ 22:45`; decision **D-31**, §11 |
...
row **O-9**. **And 7.4.3 is not mechanical after all** — the quote above is cut 25 seconds short of
the sentence that undoes it. See §12 row **O-19**; L4 (`stoic/gating.py`) deliberately does not
implement it.
...
| **D-2** | What makes a break/close "meaningful" | **Body, not wick. The stronger the surge, the better.** Confirms and narrows `M1 @ 10:03`. ~~No threshold set.~~ **Superseded on that clause by D-29, which sets one.** The qualitative half stands: D-2 is a preference ordering with no cut point, D-29 supplies the cut point. | §2.1.1, §2.3.4 |
| **D-3** | What makes a base "obvious" | **Range compression, proximity to the MA trend area, no obvious breakouts inside it.** **Superseded as the engine's definition by D-34**, which drops all three as tests — the first because ranges in a base can be about the same, the other two because **D-15**'s reset already covers them. D-3 stands as the qualitative description it always was, and as the record of what was tried. | §2.2.5 |
...
| **D-29** | **What makes a break or a close "meaningful"** — the number for both §2.1.1 and §2.3.4 | **The close must sit beyond its reference by at least 10% of the parent bar's high-low range.** Reference = the **further** MA for Step 1 (`max(sma_10, sma_20)` bullish, `min` bearish — §2.1.1 requires *both*), and the **selected boundary** for Confirmed Step 3. Parent bar per §5.2.8a / **D-23** — the nearest preceding non-inside bar, *not* `i-1`, because a run of inside bars otherwise collapses the yardstick and makes the threshold trivially satisfiable; `IBD` draws the same geometry. **Taken 2026-08-09.** The number is the human's, **chosen not measured**: the census in `docs/evidence/census_meaningful.md` enumerated 37 passages across both terms and **none quantifies either**, with `M1 @ 10:18` explicitly refusing (*"a fixed percentage or minimum candle size would create precision that the chart does not provide"*). Supersedes **D-2**'s *"No threshold set"*. Implemented in `stoic/judgment.py`, not in `stoic/sequence.py`, which stays threshold-free. **See O-17: this denominator differs from D-24's.** | §2.1.1, §2.1.4, §2.3.4 |
| **D-31** | **Does the 200 SMA rule have a bearish mirror?** — the gap §7.1.4 stated and deliberately refused to fill | **Yes. On a fast chart a short is blocked at or above the 200, exactly as a long is blocked at or below it.** The human's call on 2026-08-09: *"just like we look for longs above 50/200sma, we look for shorts below it."* `SCALP` states the bullish case only — *"on one minute chart, five minute chart, do not long into the 200 sma"* (`SCALP @ 23:02`) — and **no passage in the corpus addresses shorting into the 200 from below**, which is why §7.1.4 carried *"do not assume symmetry here"* until this row existed. So the mirror is a **strategy decision, not a reading of the material**, on the same footing as **D-29**'s 10%. **The fast-chart scoping was not part of this decision and is untouched:** `SCALP @ 22:45` licenses the higher-timeframe case outright — *"maybe on a higher time frame you can long from 10 20 back to 50, from 50 to 200"* — so the 200 gates **neither** direction when the caller declares a slow chart. **Taken 2026-08-09.** Implemented in `stoic/gating.py`; the reason member was `LONG_INTO_200` and is now `INTO_200`, which is the decision's visible edge in the code. | §7.1.4, §7.1.1 |
| **D-32** | **Which bar L4's gates and §7.1.1's confluence conditions are read on at emission** — the question `stoic/gating.py` deliberately left to its caller | **The PTB anchor bar** — the bar whose close placed or re-anchored the working order (**D-17**), i.e. the last completed non-inside bar before the fill. **The literal reading of *"present for entry"* — the fill bar's own close — is lookahead and was rejected on that ground:** **D-19** makes the gate a **close** read, while a stop-market fill is **intrabar** (§5.3.7), so gating on the fill bar's close decides risk from a number that does not exist when the order fills. Reading both the gate and the score at the anchor keeps L5 free of lookahead and makes the whole record — confidence included — available at the moment of the fill, rather than one bar late. **One consequence, recorded so it is not later mistaken for a bug:** on a **fast chart** a passing gate already implies the close is beyond **both** the 50 and the 200 in the trade direction, so §7.1.1's two inputs are **constant-true on every emitted Scalp/Day signal** and the score varies only with **D-27**'s HTF input. The three vary independently only on **slow charts** (Swing, Position), where gate 2 never fires (**D-31**). **Taken 2026-08-09.** | §7.1.1, §7.1.2, §7.1.4, §5.3.7 |
| **D-33** | **How the confluence conditions combine into `VISION.md`'s confidence score** | **A plain count of the conditions present, reported as *k* of *n*, with every condition also recorded individually on the signal record.** The inputs are the rulebook's **own already-specified conditions** read at the **D-32** bar — §7.1.1's 50 and 200 direction reads (§7.1.3 for the 200 as higher-timeframe trend) and §7.5.5 / **D-27**'s HTF confirmed Step 3 — per the human's rule on 2026-08-09, *"the rules should be present for entry."* Nothing is invented to score with. **Weights were considered and rejected:** no passage in the corpus ranks these conditions against one another, so every weight would be an invented number of exactly the kind `claude_memories/audit-hard-rules-not-in-material.md` forbids. Recording the individual flags is what lets **Phase 6** weigh them against evidence later without the score having to be re-derived. **Taken 2026-08-09.** | §7.1.1, §7.1.3, §7.5.5; `VISION.md` — *What a signal actually is* |
| **D-34** | **What makes a base "obvious"** — the last of L2's four injected predicates, and the one that stopped the engine running | **The base is the residual state, not a detected pattern: unless price is trending or breaking out, it is basing.** *Trending* is **two consecutive non-inside candles extending the same way against the parent bar** (§5.2.8a) — both extremes up, or both extremes down. Everything else is a basing candle, and the base is the **trailing run** of them, ending at the candle **before** the one being tested, floor **2 non-inside candles**, cancelled if the leg resumes. **Taken 2026-08-09**, from the human's rule: *"unless its trending or breakingout, we should think of it as a base"*, *"there could be inside bars and then it continues"*, and *"if the price is trending — as in generally making higher highs and higher lows in long and vice versa, then it is not basing."* **Three tests D-3 named were dropped rather than left open, each because something already covers it:** *range compression* — the human's call that *"ranges can be around the same"*, so there is nothing to test; *proximity to the MA trend area* — the return is toward the MAs by construction (§2.2.1) and **D-15** resets the count on a close back through both; *no obvious breakouts inside it* — the same reset. Dropping them is what keeps this decision **free of any *tuned* number**. **It is not free of numbers:** the 2-candle floor is a literal, from the human's *"min is 2 bars"*, and it is why §2.2.5a is **P** and not **M**. It lives as `MIN_BASE_CANDLES` in `stoic/judgment.py` beside `MEANINGFUL_FRACTION`, named rather than inlined, because an unnamed 2 is exactly as invisible as an unnamed 0.10. **The floor counts *non-inside* candles, and that was a correction made on review the same day:** counting positions instead let a run of inside candles be a base on its own — **96 of 413** selected bases on the first NQ replay were entirely inside candles, i.e. the *"there could be inside bars and then it continues"* case read as a Step 2. Inside candles were already skipped when *classifying*; **D-23**/§5.3.5a is the precedent for skipping them when *counting* too. The fix removed all 96 and took the replay from 246 signals to 210. **This supersedes D-3's three clauses as the engine's definition; D-3 stands as the qualitative description it always was.** **Rival constructions considered and rejected:** a **containment run** (`IBD`'s parent/inside-bar geometry) — refuted by the human, since a base may *"sweep a high or a low and then reverse"* and containment terminates at the sweep; an **overlap run**, which no passage supports; and the **pullback's terminal span**, refuted by §2.2.4 and `SCALP @ 00:28:10` (*"we have pullback to the moving averages but this is not enough to enter"*). **The corpus's loudest division on this term is dissolved, not adjudicated:** `M1 @ 20:29` waits past a *"little consolidation"* for *"the obvious"* one while `SCALP @ 00:26:10` and `PTBV @ 01:10:18` each count one as the number two (`docs/evidence/census_base_boundary.md`) — under a residual base a 2-candle base and a 20-candle base are the **same construction**, so no reading has to win. **Three structural consequences, decided here rather than left to an implementation:** the base **extends every candle** while unbroken and always ends at `pos - 1`, so the boundary is re-read from everything before the candle being tested and §2.2.8 holds literally — **note this, and not F5, is now what enforces "markable before the break"**: `base.end < pos` is vacuously true under this construction, and survives only as a guard against an *injected* predicate whose span ends at `pos`. `BASE_SELECTED` is emitted **once**, on first selection, with the span and boundary then updating silently — so **that event's span is always the 2-candle minimum and must not be read as the base that was traded**. `STEP_3_BREAK` therefore carries the **live span and the Step 2 swing** as well as the level, added on review: without them the only recorded value of **D-20**'s fib anchor would be the first-candle one, which the state that actually drove the break can contradict. And **once broken the boundary freezes**, since §2.3.1 says *"pre-selected"* and F6 treats a re-break of a still-pending base as a re-break of the **same** line — otherwise a failed break under §2.3.7 would silently move the line it failed against. Implemented in `stoic/judgment.py`; `stoic/sequence.py` gains the re-evaluation but **no threshold**. | §2.2.5, §2.2.5a, §2.2.9, §2.3.1 |
| **D-35** | **Does the leg resuming past the Step 3 extreme end an unfilled PTB walk?** | **Yes, and it ends the walk and nothing else.** Once price **trades beyond** the Step 3 High (long) / Step 3 Low (short) with the order still unfilled, the pullback is over and the working order is **cancelled**. The human's rule on 2026-08-11: *"current trade is invalidated but continuation ptb possible after that candle"*, with the standing instruction *"do not overcomplicate this."* **Three scoping choices, all taken from that sentence and stated so they are not re-derived:** an **open position is untouched** — this is not a fourth §5.4.7 condition and cannot close a trade; the **count is not reset**, so the directional state survives and §2.5 / **D-21** continuation entries stay available; and `scan_from` moves to the voiding bar, so the **new pullback must open strictly after it**, which is the same convention a fill already uses. **It is a trade-through, not a close**, matching the fill (§5.3.2) and break-even (§5.4.5) — which is precisely why it does **not** belong in §5.4.7, whose engine note says *"all three are close-based"* and that *"nothing here exits on a trade-through."* **This is the human's decision, not a reading of the material:** a scan of the 17 text sources for cancellation or invalidation language co-occurring with *step three* returns two lines, neither on this condition — same footing as **D-24**'s Step 2 boundary clause and **D-31**'s bearish mirror, per `claude_memories/audit-hard-rules-not-in-material.md`. **The geometry was checked before it was written and it bounds how often this can fire:** the trigger is a PTB extreme *inside* the pullback, so it sits between price and the Step 3 extreme, and a bar reaching that extreme has already traded through the trigger. So on a continuous bar the order **fills first** and this rule is unreachable — it bites only when a bar **gaps** through both, where the fill still wins on the open (**D-22**) unless the order was already gone. A rare rule by construction, and that was known when it was taken. Supersedes **D-17**'s *"exactly one of two ways"* on the count alone. Implemented as `ORDER_VOIDED` in `stoic/entry.py`; it adds **no threshold**. | §5.3.4a, §5.3.4b, §2.5 |
| **D-30** | **How the Step 2 boundary is selected** — does choosing the base determine the line? | **Yes: the boundary is the base's *close* extreme on the Step 1 side.** Highest close of the base for a bullish sequence, lowest for a bearish one; §2.3.1 forces the side, so the side is not a choice. **Closes, not wicks** — precedent is §7.3 / **D-8**, where `HCOM`/`LCOM` are the highest and lowest daily *close*, *"not the highest wick"*; §2.1.2's wick rule governs what counts as a **break**, not where a level is drawn. **Taken 2026-08-09.** The census in `docs/evidence/census_base_boundary.md` put the question directly and found the corpus split — **6 passages** reading boundary selection as separate from base detection, **5** reading it as one act, and **no passage addressing the relationship directly** — so this is a choice among rival readings, which is the human's under `claude_memories/audit-hard-rules-not-in-material.md`, not a derivation. **§2.2.7 is retained, not overridden:** a sloping boundary stays permitted where the base edge is a trend line, and that case is **deliberately not implemented** — see **O-18**. Default implemented as `select_boundary_from_base` in `stoic/judgment.py`; §2.2.8 is enforced structurally, since the selector only ever receives bars truncated at the base's last bar. | §2.2.6, §2.2.7, §2.2.9, §2.3.1, §5.4.7c |
...
| **O-17** | **Two different 10% rules now sit in one engine.** **D-29** confirms a break or a close when it clears its reference by 10% of the **parent bar's** range; **D-24**/§5.4.7b invalidates an open trade on a close beyond the MA by 10% of **that candle's own** range. Same number, different denominator, and neither was derived from the other — D-24 came first and D-29 chose the parent deliberately, to survive inside-bar runs. Whether the exit path should be harmonised to the parent bar, or the asymmetry is correct because entry and exit are deliberately asymmetric (§5.4.7's engine note), was **noticed rather than decided**. Same shape as **O-15**, one rule along. | §2.1.1, §2.3.4, §5.4.7b | No — both rules are computable and independently tested |
| **O-18** | **When is a base edge a *trend line* rather than a level?** **D-30** makes the boundary the base's close extreme, which is always horizontal. §2.2.7 is **retained** against it: `PC` draws the bullish boundary as an **up-sloping line over the base**, and `SCALP @ 00:05:57` narrates a sloping one live — *"we have the lower highs and the trend line holds ... when we break the boundary below."* So a sloped boundary is permitted, and nothing says when to reach for one. `Boundary` is a Protocol and `level_at(pos)` already carries any shape, so supplying one is substitution, not surgery — but **an implementation may not pick the sloping case silently**. | §2.2.6, §2.2.7 | No — **D-30**'s horizontal default is always available |
...
| **O-19** | **What "the middle of previous daily high and previous daily low" excludes.** §7.4.3 was carried as the corpus's *one mechanical* no-edge statement and was scoped into **L4** on that basis. Reading `CST` around the citation breaks it: 25 seconds earlier the same checklist asks *"Am I interacting with previous daily high, previous daily low, **previous daily close**? That's your daily level"* (`CST @ 00:22:05`–`00:22:11`), and the rule's own second clause is *"we only trade these setups when we have price traded to our **POIs**"* (`CST @ 00:22:35`). **PDC sits inside the PDH/PDL range by construction**, so the literal *strictly-between-blocks* reading forbids a trade at a level the same breath licenses. Three readings are on the table — literal close-based; intrabar *traded-to* against all three levels (`high ≥ pdh` or `low ≤ pdl` or `low ≤ pdc ≤ high`); or the hybrid — and none is stated. **The human's call on 2026-08-09 was to build neither and open this row.** `stoic/levels.py` already computes all three levels, so whichever reading wins is a substitution, not new machinery. | §7.4.3, §7.4.4 | No — L4 ships with the two MA gates and simply filters less |
| **O-20** | **§2.2.9's tie-break has no counterpart in the engine, and that was not decided — it fell out.** The rule is *"if two boundaries look equally valid there is no clean step three yet"* — **wait**. Under **D-34** the base is the trailing run of basing candles, so **exactly one span exists at any bar** and two candidates can never be present to tie. The engine therefore never waits on this ground. Two readings, neither stated: the tie-break is **satisfied vacuously**, because a residual base is by construction the one that *"clearly controls"*; or it is **silently dropped**, and `M1 @ 17:59`/`@ 18:22`'s nested bases (*"you can try to enter on this or you wait for the larger retest larger base"*) are a real choice the engine no longer makes. **Do not add a selector to fill this** — that is the invention `claude_memories/audit-hard-rules-not-in-material.md` forbids. Noticed, not decided, same shape as **O-15**, **O-17** and **O-18**. | §2.2.9, §2.2.5a | No — a residual base never yields **two** spans, so there is nothing to tie. It yields **none** often (the run is short, or the leg is trending), and waiting on *that* is §2.2.5a working |
