...
| Fill in one of **L2's four injected predicates** — "meaningful" break, obvious base, boundary selection, "meaningful close" | `docs/STATE.md` — they are routed to Phase 4's SLM (user's call, 2026-08-09); `docs/RULEBOOK.md` §0 (a number for a **J** term is a strategy decision requiring the human), §2.1.4, §2.2.5–§2.2.9, §2.3.4, **D-2**, **D-3**, and **D-9** for the propose-then-confirm pattern. The SLM proposes, the human confirms, it lands as a **D-row in §11**. **If a term cannot be grounded, it comes back for a decision — it never gets a default.** An unfilled predicate is the correct state |
