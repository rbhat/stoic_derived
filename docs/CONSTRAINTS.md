...
| Emit a signal record | `VISION.md` — "What a signal actually is"; and `docs/RULEBOOK.md` §5.3.10's engine note, which adds a field the schema omits: store the **trigger and the fill**, because they differ on a gap and only the fill sets R (§5.4.2) |
| **Compute the confidence / confluence score** | `docs/STATE.md` first — this is **L5's trap**. `VISION.md` requires it deterministic and rules-computed, and `docs/RULEBOOK.md` names exactly **one** input (**D-27**, §7.5.5 — HTF alignment raises it and never blocks) and **no** weights, inputs or scale. Picking a set and weighting it is what `claude_memories/audit-hard-rules-not-in-material.md` forbids, and any choice will look reasonable. **It goes to the human before it is written** |
