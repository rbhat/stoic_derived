...
- [OHLC frame-to-bar join](ohlc-frame-to-bar-join.md) — the chart header joins to one bar on **3-of-4 legs**, validated against a perturbed control (0/44 match); and the counting trap — **232 OHLC frames are 44 distinct quadruples**, so count distinct, never frames
