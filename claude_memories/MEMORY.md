...
- [WP-V §3.1 visual harvest status](visual-harvest-31-status.md) — **active work.** 10,120 visual states harvested + audited; dHash alone under-splits; A2 is the one open check and the check itself is wrong; §3.2 VLM not started
...
- [SLM model artifacts](slm-model-artifacts.md) — where the fine-tuned Qwen3-8B lives (on the Mac it is under `artifacts/`, no dot, no runner installed); retrain decision **REOPENED** 2026-07-26, sequenced after the visual re-extraction
